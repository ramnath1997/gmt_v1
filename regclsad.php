...
<html>
    <head>
        <title>Class Advisor Registration Portal</title>
    ...
    <body>
        ...
        <div class="main-agileits">
            <h1>Class Advisor Registration</h1>
...
<form name="ca_reg" method="POST">

										<input type="hidden" name="ca_reg"   value="yes" />
										<p>Select Batch<p>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" )
	{
		if(isset($_POST['ca_reg']))
		{
			if(trim($_POST['ca_reg'])=="yes")
			{

							if( isset($_POST['student_year'])  AND isset($_POST['student_course']) AND isset($_POST['ca_id']) AND isset($_POST['ca_password']) AND isset($_POST['student_dept']) AND isset($_POST['student_sem']) )
							{
								if( trim($_POST['student_year'])!=""  AND trim($_POST['student_course'])!="0" AND trim($_POST['student_dept'])!="0" AND trim($_POST['ca_id'])!="" AND trim($_POST['ca_password'])!="" AND trim($_POST['student_sem'])!="0" ){
								...
								{
									$sql="SELECT ca_id FROM class_advisor WHERE ca_id='$ca_id'";
									...
									mysql_query("INSERT INTO class_advisor (ca_id, st_code, type, password, dept, access) VALUES ('{$ca_id}','{$st_code}','{$type}','{$ca_password}','{$s_dep}', '0') ");
									$t='1';
...
if($t=='1')
	echo "<p><br>Registered Successfully<p>";
if($count==1)
				{
					echo "<p><br>ID already Used<p>";
				}
?>
									...
									if(trim($_POST['revoke'])=="Revoke")
									{
										...
										if(isset($st_code)){
											$w=$st_code;
											 ...
											 $sql="SELECT student_regno FROM table_student_details WHERE st_code='$w' AND dept='$staff_dept' ";
											$result=mysql_query($sql) or die (mysql_error());
				 							$count=mysql_num_rows($result);
											if($count>0)
											{
												$c='1';
                    							mysql_query("UPDATE table_student_details SET access='1' WHERE st_code='$w' AND dept='$staff_dept'");
                    							mysql_query("UPDATE class_advisor SET access='1' WHERE st_code='$w' AND dept='$staff_dept'");
        						...
        						if(isset($_POST['recall']))
        						{
            						if(trim($_POST['recall'])=="Recall")
            						{
                						if(isset($st_code)){
											...
											{
												$c='1';
                    							mysql_query("UPDATE table_student_details SET access='0' WHERE st_code='$w' AND dept='$staff_dept'");
                    							mysql_query("UPDATE class_advisor SET access='0' WHERE st_code='$w' AND dept='$staff_dept'");
                    							echo "<p>Class Recalled</p>";
											}
											else{
												echo "<p>No Records</p>";
